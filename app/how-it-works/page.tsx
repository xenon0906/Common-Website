...
function RealTimeSection({ mockupUrl }: { mockupUrl: string }) {
...
function ScheduledSection({ mockupUrl }: { mockupUrl: string }) {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, margin: '-100px' })
...
export default function HowItWorksPage() {
  ...
  return (
    <SiteLayout>
      <HeroSection />
      <RealTimeSection mockupUrl={images.mockups.homeScreen} />
      <ScheduledSection mockupUrl={images.mockups.createTrip} />
      <ComparisonSection />
      <CTASection />
    </SiteLayout>
  )
