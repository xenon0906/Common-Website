...
// Founder-specific display enhancements (icon references can't be stored in Firestore)
const founderEnhancements: Record<string, {
...
// Hero section
function HeroSection({ teamCount }: { teamCount: number }) {
  return (
...
// Individual founder section (rich layout)
function FounderSection({
  ...
  const enhancement = founderEnhancements[member.name]
  const gradient = enhancement?.gradient || 'from-primary via-blue-500 to-teal-500'
    ...
    >
      <div className="container mx-auto px-4 xs:px-6 sm:px-8 md:px-12 lg:px-16 xl:px-20 2xl:px-24">
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-16 items-center ${isReversed ? 'lg:grid-flow-dense' : ''}`}>
          ...
          >
            <div className={`relative aspect-[4/5] max-w-md mx-auto rounded-3xl overflow-hidden bg-gradient-to-br ${gradient}`}>
              {member.imageUrl ? (
                <Image
                  src={member.imageUrl}
                  alt={member.name}
            ...
            <div className="max-w-xl">
              ...
              {enhancement?.highlights && (
                <div className="space-y-4">
                  {enhancement.highlights.map((highlight, idx) => {
              ...
              {!enhancement?.highlights && member.bio && (
...
// Team members grid (for non-founder members)
function TeamMembersGrid({ members }: { members: TeamMemberData[] }) {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, margin: '-100px' })
      ...
      <div className="container mx-auto px-4 xs:px-6 sm:px-8 md:px-12 lg:px-16 xl:px-20 2xl:px-24">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            The People Behind{' '}
            <span className="text-primary">Snapgo</span>
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {members.map((member, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 text-center">
                {/* Avatar */}
                <div className="relative w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden bg-gradient-to-br from-primary/20 to-teal-500/20">
                  {member.imageUrl ? (
                    <Image
                      src={member.imageUrl}
                      alt={member.name}
                ...
                {member.linkedin && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 mt-4 text-[#0A66C2] hover:text-[#004182] text-sm font-medium transition-colors"
                  >
...
function ValuesSection() {
  ...
  const values = [
    { icon: Heart, title: 'User First', description: 'Every decision starts with our users. Their safety, savings, and satisfaction drive us.', color: 'from-pink-500 to-rose-500' },
    { icon: Target, title: 'Innovation', description: 'We challenge the status quo and seek better solutions for urban mobility.', color: 'from-primary to-blue-500' },
    { icon: Sparkles, title: 'Transparency', description: 'Open communication, honest pricing, and clear expectations build trust.', color: 'from-teal-500 to-emerald-500' },
    { icon: Rocket, title: 'Impact', description: 'We measure success by the real difference we make in lives and the environment.', color: 'from-violet-500 to-purple-500' },
  ]
...
export default function TeamPageClient({ team }: TeamPageClientProps) {
  // Split into founders (first 3 with enhancements) and other members
  const founders = team.filter(m => founderEnhancements[m.name])
  const otherMembers = team.filter(m => !founderEnhancements[m.name])

  return (
    <>
      <HeroSection teamCount={team.length} />

      {/* Founder Sections (rich layout) */}
      ...
      <TeamMembersGrid members={otherMembers} />
