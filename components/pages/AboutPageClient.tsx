...
import { AboutContentData, TeamMemberData, SiteConfigData } from '@/lib/content'
...
interface AboutPageClientProps {
  about: Record<string, AboutContentData>
  team: TeamMemberData[]
...
function FounderStory({ about }: { about: Record<string, AboutContentData> }) {
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="text-gray-600 text-lg leading-relaxed mb-6">
                {about.origin?.content || "It was a regular day when we, Mohit and Surya Purohit, were heading to Ghaziabad Railway Station from our society. We booked a cab and noticed another person also taking a cab from our area. When we reached the station, we saw the same person at the parking lot. That's when it hit us - we both paid ₹300 separately for the same route."}
              </p>

              <p className="text-gray-600 italic mb-6">
                "{about.spark?.content || "This sparked an idea, but we didn't want to do traditional carpooling with private cars — that's not legal for commercial use and bypasses taxi drivers who depend on fares. Instead, we pioneered 'Cab Pooling' — pooling commercial cabs among verified riders."}"
              </p>
...
function MissionVisionValues({ about }: { about: Record<string, AboutContentData> }) {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, margin: '-100px' })
  ...
  const items = [
    ...
    },
    {
      icon: Heart,
      title: 'Values',
      content: about.values?.content || 'Legal and ethical operations, driver-friendly ecosystem, environmental sustainability.',
...
// Team CTA Section
function TeamCTA({ siteConfig }: { siteConfig: SiteConfigData }) {
...
export function AboutPageClient({ about, team, siteConfig }: AboutPageClientProps) {
  return (
    <SiteLayout>
      <AboutHero />
      <VideoDocumentarySection />
      <FounderStory about={about} />
      <MissionVisionValues about={about} />
      <Timeline />
      <TeamCTA siteConfig={siteConfig} />
    </SiteLayout>
  )
}
